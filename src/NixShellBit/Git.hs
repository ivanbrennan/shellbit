...
gitDiscoverRepo startPath ceilingDirs =
  ...
  where
    ...
    discover
      ...
      -> Ptr CChar
      -> IO (Maybe FilePath)
    discover start_path ceiling_dirs path_out = do
      exit <- c'git_repository_discover
                path_out path_size start_path across_fs ceiling_dirs

      case exit of
        e | e == c'GIT_OK        -> Just <$> peekCString path_out
          | e == c'GIT_ENOTFOUND -> pure Nothing
